...
img_size = (160, 160)  # 모델에 입력되는 영상 크기
...
def make_model(img_size, num_classes):  # U-net 모델을 만드는 클래스
    ...
    for filters in [256, 128, 64, 32]:  # U-net의 업 샘플링(확대 경로)
        x = layers.Activation('relu')(x)
...
train_gen = OxfordPets(batch_size, img_size, train_img_paths, train_label_paths)  # 훈련 집합
test_gen = OxfordPets(batch_size, img_size, test_img_paths, test_label_paths)  # 검증 집합

model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
cb = [keras.callbacks.ModelCheckpoint('oxford_seg.h5', save_best_only=True)]  # 체크포인트 기능을 이용하여 학습 도중에 모델을 저장

model.fit(train_gen, epochs=30, validation_data=test_gen, callbacks=cb)  # 학습 도중에 발생한 가장 높은 성능의 모델만 기록

preds = model.predict(test_gen)

cv.imshow('Sample image', cv.imread(test_img_paths[0]))  # 예측 결과를 디스플레이
cv.imshow('Segmentation label', cv.imread(test_label_paths[0]) * 64)
cv.imshow('Segmentation prediction', preds[0] * 64)
